...
/** The host a client dials: instance domain, else a concrete bind host, else a placeholder. */
function remoteHost(config: RatholeConfig): string {
...
function clientGlobalLines(config: RatholeConfig): string[] {
  const { port } = splitHostPort(config.bindAddr);
  const out = ["[client]", `remote_addr = ${q(`${remoteHost(config)}:${port || "2333"}`)}`];
...
export function generateClientGlobalToml(config: RatholeConfig): string {
  return (
    ["# rathole client — global section. Run with `rathole client.toml`.", "", ...clientGlobalLines(config)]
      .join("\n")
      .trimEnd() + "\n"
  );
}
 ...
 */
export function generateClientToml(config: RatholeConfig): string {
  const out = [
